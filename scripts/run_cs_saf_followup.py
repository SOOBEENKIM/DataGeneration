...
def validate_cpu():
    path=OUTPUT/'cpu_v3/COMPLETE.json';r=json.loads(path.read_text())
...
def main():
    ...
    if args.phase=='cpu':
        r=cpu_gate(OUTPUT/'cpu_v3');print(json.dumps(r),flush=True)
        ...
        write_json(OUTPUT/'COMPLETE.json',{'status':'COMPLETE','source_commit':frozen_source(),'config_sha256':CONTRACT_SHA,
            ...
            'cpu_gate_path':str(OUTPUT/'cpu_v3/COMPLETE.json'),'cpu_gate_sha256':sha256(OUTPUT/'cpu_v3/COMPLETE.json'),'gpu_progress_sha256':sha256(OUTPUT/'gpu_progress.json'),'generation_progress_sha256':sha256(OUTPUT/'generation_progress.json')})
    else:
