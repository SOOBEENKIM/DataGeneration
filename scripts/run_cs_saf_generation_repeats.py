"""Run same-source gates and the registered 800 frozen-model generations."""
...
from experiments.cs_saf_generation_repeats import (ROOT,OUTPUT,CONFIG_SHA,contract,
    run_cell,frozen_source,smoke,verify,folder_for)
...
def run():
    ...
    check_gate('cpu_gate',source);check_gate('gpu_gate',source);check_gate('metric_gate',source)
    tasks=[(pi,k,t) for pi in c['prevalences'] for t in c['trials'] for k in c['kappas']]
    pending=[];completed=[];running={};failures=[];allocations=[]
    for task in tasks:
        pi,k,t=task;folder=folder_for(pi,k,t)
    ...
    while pending or running:
        for uuid,(proc,handle,task) in list(running.items()):
            ...
            if status:failures.append(dict(task=task,exit_code=status))
                ...
                pi,k,t=task;item=verify(folder_for(pi,k,t))
        ...
        if pending and len(running)<c['max_workers']:
            for gpu in available_gpus():
                ...
                task=pending.pop(0);pi,k,t=task
                handle=(logs/f'pi_{pi:.2f}_k{k}_t{t}.log').open('x')
                ...
                command=[sys.executable,'-m','scripts.run_cs_saf_generation_repeats','worker',
                    '--pi',str(pi),'--kappa',str(k),'--trial',str(t),'--device','cuda:0']
    ...
    write_json(OUTPUT/'GRID_COMPLETE.json',dict(source_commit=source,config_sha256=CONFIG_SHA,
        completed=completed,new_generated_datasets=20*len(completed),base_model_fits=0,calibration_fits=0,test_accessed=False))
...
def main():
    ...
    if a.phase=='cpu':
        ...
        source=frozen_source();check_gate('cpu_gate',source);check_gate('gpu_gate',source);check_gate('metric_gate',source)
        c=contract()
        if a.pi not in c['prevalences'] or a.kappa not in c['kappas'] or a.trial not in c['trials']:
            raise ValueError('unregistered cell')
        run_cell(a.pi,a.kappa,a.trial,torch.device(a.device))
    else:run()
