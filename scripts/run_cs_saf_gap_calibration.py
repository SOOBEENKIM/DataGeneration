"""Run 80 equal gap-correction fits and 400 registered generations on idle GPUs."""
...
from experiments.cs_saf_gap_calibration import (ROOT,OUTPUT,CONFIG_SHA,contract,
    fit_job,frozen_source,smoke,verify,folder_for)
...
def run():
    ...
    tasks=[(pi,k,t,n) for pi in c['prevalences'] for t in c['trials'] for k in c['kappas'] for n in c['parents']]
    pending=[];completed=[];running={};failures=[];allocations=[]
    for task in tasks:
        pi,k,t,n=task;folder=folder_for(pi,k,t,c['variants'][n])
    ...
    while pending or running:
        for uuid,(proc,handle,task) in list(running.items()):
            ...
            if status:failures.append(dict(task=task,exit_code=status))
                ...
                pi,k,t,n=task;item=verify(folder_for(pi,k,t,c['variants'][n]))
        ...
        if pending and len(running)<c['max_workers']:
            for gpu in available_gpus():
                ...
                task=pending.pop(0);pi,k,t,n=task
                handle=(logs/f'pi_{pi:.2f}_k{k}_t{t}_{n}.log').open('x')
                ...
                command=[sys.executable,'-m','scripts.run_cs_saf_gap_calibration','worker',
                    '--pi',str(pi),'--kappa',str(k),'--trial',str(t),'--parent',n,'--device','cuda:0']
    ...
    write_json(OUTPUT/'GRID_COMPLETE.json',dict(source_commit=source,config_sha256=CONFIG_SHA,
        completed=completed,new_generated_datasets=5*len(completed),base_model_fits=0,calibration_fits=len(completed),test_accessed=False))
...
def main():
    ...
    p.add_argument('--kappa',type=int);p.add_argument('--trial',type=int);p.add_argument('--parent',choices=['Ucal','Ecal'])
    a=p.parse_args();c=contract();OUTPUT.mkdir(parents=True,exist_ok=True)
    torch.set_num_threads(1);torch.set_num_interop_threads(1)
    from experiments.cof_seqgen_saf_training import _seed_everything
    _seed_everything(20260920)
    torch.backends.cuda.matmul.allow_tf32=c['matmul_allow_tf32'];torch.backends.cudnn.allow_tf32=c['cudnn_allow_tf32']
    ...
    if a.phase=='cpu':
        ...
        fit_job(a.pi,a.kappa,a.trial,a.parent,torch.device(a.device))
    else:run()
