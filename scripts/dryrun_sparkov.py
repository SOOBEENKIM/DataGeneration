"""
Step 0 (Sparkov): Single-config end-to-end dry run.

Config: λ=1.0 · keep=0.1 · seed=1 · T_diff=50 DDIM steps

Sparkov role in C3: augmentation headline (ΔAUPRC).
  Scarcity headroom exists: ~0.75 (scarce-real) vs ~0.78 (full data).
  CoF(λ>0) goal: push synthetic-augmented AUPRC toward/above full.

Pipeline (same as AMLSim dryrun):
  Phase 0  — scarce split (stratified keep=0.1)
  Phase 1  — fixed g_std from scarce train
  Phase 2  — pretrain BehaviorTeacher (300 steps, normalized g)
  Phase 3  — train CoF-SeqGen (n_steps; log L_diff, L_coh, ||num_grad||)
  Phase 4  — DDIM generation (conditional: real dt_bin + x_cat, generate x_num)
             Generation quality gate: marginal distributions real vs synthetic
  Phase 5  — TSTR evaluation
             Features: amt stats + dt_bin stats + merchant-cat stats + seq_len
             HGB(scarce real)       → AUPRC_base
             HGB(synthetic)         → AUPRC_synth
             HGB(real+synth augmented) → AUPRC_aug
  Phase 6  — Coherence gap: |P(fraud|high_vel)_synth − P(fraud|high_vel)_real|

Purpose: verify evaluation harness + confirm headroom exists before λ sweep.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Training loop (same as AMLSim dryrun)
# ─────────────────────────────────────────────────────────────────────────────

def train_cof(
    model, x_num, dt_bin, x_cat, y, mask,
    n_steps, batch_size, lr, log_every, device,
) -> dict:
    model.train()
    model.to(device)
    ...
    N = x_num.shape[0]
    history = {'L_diff': [], 'L_coh': [], 'L_label': [], 'num_grad_norm': [], 'step': []}
    t_start = time.time()
...
def generate_synthetic(model, dt_bin_cond, x_cat_cond, d_num, T_diff, batch_size, device, valid_mask):
    """Conditional generation: fixed dt_bin/x_cat, generate x_num via DDIM."""
    model.eval()
...
def generation_quality_gate(
    ...
    """
    Compare marginal distributions of real vs synthetic.
    PASS if mean difference < sigma_threshold × real_std for all columns.
    ...
    for c, name in enumerate(col_names):
        r = x_num_real[:, :, c][bool_mask].flatten()
        s = x_num_synth[:, :, c][bool_mask].flatten()
        mean_diff = abs(r.mean() - s.mean())
        gate_thr  = sigma_threshold * r.std()
        ok = mean_diff < gate_thr
        if not ok:
            passes = False
        flag = '✓' if ok else '✗'
        print(f'  {flag} {name:10s}: '
              f'real mean={r.mean():.3f} std={r.std():.3f} '
              f'| synth mean={s.mean():.3f} std={s.std():.3f} '
              f'| Δmean={mean_diff:.3f} (thr={gate_thr:.3f})')
    print(f'  Gate: {"PASS ✓" if passes else "FAIL ✗ — increase n_steps or check sampling"}')
    return passes
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  Sparkov Dry Run | λ={args.coh_lambda} | keep={args.keep} | seed={args.seed}')
    ...
    W         = 60.0          # 1-hour burst window (vel_1h ablation 14× lift)
    temp      = 1.0
    d_num     = meta['d_num']
    ...
    scarce_idx = stratified_keep(len(y_seq_full), y_seq_full, keep=args.keep, seed=args.seed)
    ...
    print(f'  Note: gap scale = {g_std[1]:.1f} (tau_k max=2323min, W=60min)')
    ...
    history = train_cof(
        ...
        lr=args.lr, log_every=args.log_every, device=device,
    )

    # ── Phase 4: Generate + Quality Gate ─────────────────────────────────────
    print_section(f'Phase 4 — DDIM generation (T={args.T_diff} steps) + Quality Gate')
    ...
    gate_pass = generation_quality_gate(
    ...
    X_full_tr, y_full_lab = extract_features(
        x_num_full.numpy(), dt_bin_full.numpy(), mask_full.numpy(),
        y_full.numpy(), x_cat_full.numpy()
    )
    result_full = run_tstr(X_full_tr, y_full_lab, X_test, y_test, random_state=args.seed)

    # Synthetic only
    X_synth, y_synth_lab = extract_features(
        x_num_synth.numpy(), dt_bin.numpy(), mask.numpy(),
        y.numpy(), x_cat.numpy()           # real dt_bin/x_cat, real y labels
    )
    ...
    delta_synth = result_synth['auprc'] - result_base['auprc']
    delta_aug   = result_aug['auprc']   - result_base['auprc']
    headroom    = result_full['auprc']  - result_base['auprc']

    print(f'\n  Test set: {int(y_test.sum())} fraud / {len(y_test)} seqs '
          f'[prev {y_test.mean():.3%}]')
    print(f'  {"─"*50}')
    print(f'  Full-train (upper bound)  AUPRC = {result_full["auprc"]:.4f}')
    print(f'  Scarce-real (baseline)    AUPRC = {result_base["auprc"]:.4f}')
    print(f'  Headroom (full−base)            = {headroom:+.4f}')
    print(f'  {"─"*50}')
    print(f'  Synthetic only            AUPRC = {result_synth["auprc"]:.4f}  Δ={delta_synth:+.4f}')
    print(f'  Real scarce + synthetic   AUPRC = {result_aug["auprc"]:.4f}   Δ={delta_aug:+.4f}')

    saturation_ok = headroom > 0.01
    if not saturation_ok:
        print(f'  ⚠  headroom={headroom:.4f} — task may be saturated at keep={args.keep}')
    else:
        print(f'  ✓  headroom={headroom:.4f} — augmentation scenario valid')

    # ── Phase 6: Coherence gap ────────────────────────────────────────────────
    print_section('Phase 6 — Coherence gap (P(fraud|behavior) coupling)')
    ...
    bool_mask_np = mask.numpy().astype(bool)
    ...
    g_real_ent  = seq_mean_g(g_real_np,  bool_mask_np)
    g_synth_ent = seq_mean_g(g_synth_np, bool_mask_np)

    feat_names = ['vel', 'gap', 'rep', 'amt_sum']
    for fi, fname in enumerate(feat_names):
        raw_std = g_real_ent[:, fi].std()
        if raw_std < 0.01:
            print(f'  [skip] {fname}: degenerate (std={raw_std:.4f})')
            continue
        ...
        print(f'  coherence_gap[{fname}] = {gap_val:.5f}')

    # ── Summary ───────────────────────────────────────────────────────────────
    print(f'\n{"="*60}')
    print(f'  SUMMARY — Sparkov λ={args.coh_lambda} keep={args.keep} seed={args.seed}')
    print(f'  g_std: vel={g_std[0]:.2f} gap={g_std[1]:.2f} '
          f'rep={g_std[2]:.2f} amt={g_std[3]:.2f}')
    ...
    print(f'  Quality gate: {"PASS" if gate_pass else "FAIL — more steps needed"}')
    print(f'  Headroom: {headroom:+.4f} '
          f'({"OK" if saturation_ok else "saturated — reduce keep"})')
    print(f'  ΔAUPRC (synth−base) = {delta_synth:+.4f}')
    print(f'  ΔAUPRC (aug−base)   = {delta_aug:+.4f}')
    proceed = gate_pass and saturation_ok
    print(f'  → Proceed to λ sweep: {"YES" if proceed else "NO — see above"}')
    print(f'{"="*60}\n')
