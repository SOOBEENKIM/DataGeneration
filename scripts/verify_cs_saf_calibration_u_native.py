...
from experiments.cs_saf_calibration_u import OUTPUT,contract,parent
...
def main():
    ...
    for pi in c['prevalences']:
        for k in c['kappas']:
            ...
            for t in c['trials']:
                for name in ('U','Ucal','E','Ecal'):
                    if name.endswith('cal'):
                        root=OUTPUT if name=='Ucal' else ROOT/'artifacts/cs_saf/calibration_v1'
                        folder=root/f'pi_{pi:.2f}/kappa_{k}/trial_{t}/{name}'
