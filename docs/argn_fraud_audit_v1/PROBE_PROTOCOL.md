...
과거 A의 teacher 결과를 본 뒤 fit-only merchant→최빈 category 조회를 추가했다. validation 어휘나 정답으로 조회표를 만들지 않는다. 이는 동일 데이터의 행 관계가 단순 조회로 얼마나 설명되는지 보는 검사이며, 사전 등록된 주요 지표나 완전한 순차 생성 baseline이 아니다. 이전 probe JSON의 protocol hash는 이 문단이 추가되기 전 문서를 가리킨다.
