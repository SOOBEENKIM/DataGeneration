...
후속 [Sparkov 통제 대조](../sparkov_argn_control_v2/README.md)는 Berka에서 확인한
engine 1.0.4로 새로 실행했다. 전처리·학습 상한·생성 순서와 잔여 사기 흐름
오류를 구분한다. 아래 engine 2.4 결과는 별도 조건의 이전 기록으로 보존한다.

**2026-09-27 Berka 대조 완료:** 아래 Sparkov 감사 전에 원문 기준을 확인했어야 했다. 이를 바로잡아 저자 공개 생성물 5회 재평가와 원본 Berka 새 학습·생성 1회를 완료했다. 새 결과는 overall **.785**, coherence **.815**로 원문 범위 안이다. [실행 결과와 재현 범위](BERKA_REPLICATION_RESULTS.md), [우선순위 정정 기록](PAPER_REPRODUCTION_CORRECTION.md). 이전 Sparkov는 [같은 설정의 비교 실험이 아니므로](BERKA_SPARKOV_COMPARABILITY.md), 그 오류를 곧바로 논문 재현 실패로 해석하지 않는다.
