...
> **문서 우선순위:** 이 문서는 실행 전의 넓은 후보 메모다. 공식 모델 실행 후의 최신 판단과 더 좁힌 검증 구조는 [외부 확인 후 연구 결정](external_audit_v1/research_decision.md)을 따른다. 아래의 전체 gap–mark 결합을 최종 채택한 것으로 해석하지 않는다.
