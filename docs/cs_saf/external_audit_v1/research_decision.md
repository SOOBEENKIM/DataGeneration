...
**최신 실행 상태:** 아래에서 제안했던 전체 신경망은 이후 [동일 조건 U/G/C 비교](../structure_v1/README.md)로 구현·학습·평가했다. 18회 학습과 162개 생성 데이터셋에서 C는 일반 구조 G와 U+gap보다 활성 예측·생성 오차가 커서 사전 기준을 통과하지 못했다. 현재 C를 최종 구조로 채택하거나 확대하지 않는다. 아래의 미구현/미실행 표현은 설계 당시 기록으로 보존하며, 실제 최신 상태·주장은 후속 보고서에 있다.
